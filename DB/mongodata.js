// use fluttercourse; db create

//para tutores
tutor1= {
	"nombre" : "Nicole Pamela",
	"ape_pat" : "Garcia",
    "ape_mat" : "Cano",
    "rol":"tutor",
	"cursos" : [
		{
			"nombre" : "Flutter basico",
			"horario" : [
				{
					"dia" : "miercoles",
					"hora_ini" : "21:00:00",
					"hora_fin" : "23:00:00"
				}
			]
		}
	]
}

tutor2= {
	"nombre" : "Samuel Benjamin",
	"ape_pat" : "Cruz",
    "ape_mat" : "Quino",
    "rol":"tutor",
	"cursos" : [
		{
			"nombre" : "Flutter basico",
			"horario" : [
				{
					"dia" : "miercoles",
					"hora_ini" : "21:00:00",
					"hora_fin" : "23:00:00"
				}
			]
		}
	]
}

tutor3= {
	"nombre" : "Adalid",
	"ape_pat" : "Limachi",
    "ape_mat" : "Limachi",
    "rol":"tutor_admin",
	"cursos" : [
		{
			"nombre" : "Flutter basico",
			"horario" : [
				{
					"dia" : "miercoles",
					"hora_ini" : "21:00:00",
					"hora_fin" : "23:00:00"
				}
			]
		}
	]
}

db.tutores.insert(tutor1);
db.tutores.find();


//para estudiantes

estudiante1= {
    "nombre":"gary",
    "ape_pat":"ap12",
    "ape_mat": "apm12",
    "sexo": true,
    "email":"estudian12@gmail.com",
    "telefono":1243543453,
    "institucion":"UMSA",
    "asistencia":[
        {
            "fecha_asiste": "12/12/12",
            "curso":"Flutter Basico",
            "historial":[
                {
                    "tutor":"Benjamin Cruz Quino",
                    "descripcion":"tomar lista"
                }
            ]
        }
    ]
}
db.estudiante.insert(estudiante1);

//cursos
curso1 ={
    "nombre" : "Flutter basico",
    "horario" : [
        {
            "dia" : "miercoles",
            "hora_ini" : "21:00:00",
            "hora_fin" : "23:00:00",
            "gestion":"2020"
        }
    ],
    "estudiantes":[{
        "nombre":"alvin",
        "ape_pat":"ap",
        "ape_mat": "apm",
        "email":"estudian@gmail.com",
        "telefono":1234553
    }],
    "files":[
        {
            "documentos":[
                {
                    "filename":"file1",
                    "descripciojn":"loremimpsun"
                }
            ],
            "imagenes":[
                {
                    "url":"data:image/png;base64,",
                    "descripcion":"lorem"
                }
            ]
        }

    ],
    
    

}